...
# obj1 = soup.select('div[class=r_lbx] div[class=r_lbx_cen] div[class=r_lbx_cena] a')
# obj2 = soup.select('div[class=r_lbx] div[class=r_lbx_cen] div[class=r_lbx_cenb]')
# obj3 = soup.select('div[class=r_lbx] div[class=r_lbx_money] div[class=r_lbx_moneya] span[class=ty_b]')
# for item1, item2, item3 in zip(obj1, obj2, obj3):
#     print(item1.attrs['title'], end=' ')
#     obj_s = item2.get_text().strip()
#     obj_list = obj_s.split('|')
#     for i in obj_list:
#         print(i.strip(), end=' ')
#     print(item3.get_text().strip())
import csv
import random
import re
...
def getdata(city='hz', district='西湖区'):
    unicode_district = quote(district)
    ...
    for i in range(1, 30):
        print("%s第%s页------------------------" % (district, i))
        # 拼接url
        url = 'https://www.danke.com/room/%s/d%s.html?page=%s' % (city, unicode_district, i)
        try:
        ...
        with open('job.csv', 'a+', newline='') as f:
            ...
            for item1, item2, item3 in zip(obj1, obj2, obj3):
                # print("aaaa-----------")
                # print(item1.attrs['title'], end=' ')
                one_list = ['---', '上海', district, '---', '---', '---', '---', '---', '---', '---',
                            '---', '---', '押一付三', '---']
                d_list = item1.attrs['title'].split(' ')
                one_list[3] = d_list[2]
                one_list[4] = d_list[3]
                ...
                obj_list = obj_s.split('|')
                try:
                    mianji = re.findall(r'建筑面积约(\d+)㎡', obj_list[0].strip())[0]
                except:
                    print("面积fail")
                    mianji = 12
                one_list[6] = mianji
                low = int(mianji) - int(mianji) % 10
                one_list[7] = "(%s, %s]" % (low, low + 10)

                price = item3.get_text().strip()
                one_list[8] = price
                one_list[9] = price
                one_list[10] = price
                one_list[11] = price
...
def createfile():
    fieldsname = ['小区ID', '城市', '行政区', '小区名称', '户型分类', '户型类型', '标准面积', '面积区间',
                  '户型租金/毛坯', '租金/简易装修', '租金/精装修', '租金/豪华装修', '租金/付款方式', '更新时间']
...
if __name__ == '__main__':
    city = 'sh'
    district = '闵行区'
    district_list = ['浦东新区', '长宁区', '虹口区']
    # createfile()
    for d in district_list:
        getdata(city=city, district=d)
